...
function tree(treeConf) {
	...
	var that = this;
	var plotLines = treeConf.functionTree;
	var plotLineExceptions = treeConf.plotLines;
	var depth = 0;
	var absMaxDepth = 0;
	// Private methods
	
	var traverse = function (pL, depth, defaultName){
		...
		if (pL.hasOwnProperty("children")) {
			...
			for (var i=0; i<pL.children.length; i++) {
				...
				child.parent = pL;
				traverse(child, depth+1, defaultName);
	...
	this.master = this.plotLineDesc[0];
